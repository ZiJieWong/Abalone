...
#Add new column for abalone age group
baoyudata3$age_group <- NA

#Define the age groups for abalone
young <- c(0, 7.5)
adult <- c(7.5,14.5)
old <- c(14.5, Inf)

# Group the data in the 'age' column into the specified age groups
baoyudata3$age_group[baoyudata3$age < young[2]] <- "young"
baoyudata3$age_group[baoyudata3$age >= adult[1] & baoyudata3$age < adult[2]] <- "adult"
baoyudata3$age_group[baoyudata3$age >= old[1]] <- "old"
